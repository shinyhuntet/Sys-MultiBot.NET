...
public abstract class PokeRoutineExecutor8SWSH(PokeBotState cfg) : PokeRoutineExecutor<PK8>(cfg)
{
    ...
    public async Task StartGameLair(PokeTradeHubConfig config, CancellationToken token)
    {
        // Open game.
        ...
        Log("Restarting the game!");
        // Switch Logo lag, skip cutscene, game load screen
        await Task.Delay(config.Timings.ExtraTimeLoadGame, token).ConfigureAwait(false);


        var heap = await SwitchConnection.GetHeapBaseAsync(token).ConfigureAwait(false);
        var main = await SwitchConnection.GetMainNsoBaseAsync(token).ConfigureAwait(false);
        while (heap == 0 || main == 0)
        {
            await Task.Delay(1_000, token).ConfigureAwait(false);
            Log("Waitng");
            if (heap == 0)
                heap = await SwitchConnection.GetHeapBaseAsync(token).ConfigureAwait(false);
            if (main == 0)
                main = await SwitchConnection.GetMainNsoBaseAsync(token).ConfigureAwait(false);
        }
        while (!await IsOnTitle(token).ConfigureAwait(false))
        {
            Log("waiting for the right pointer...");
    ...
    public async Task<bool> IsOnTitle(CancellationToken token)
    {
        var (valid, offset) = await ValidatePointerAll(Offsets.OverworldPointer, token).ConfigureAwait(false);
        if (!valid)
            return false;
