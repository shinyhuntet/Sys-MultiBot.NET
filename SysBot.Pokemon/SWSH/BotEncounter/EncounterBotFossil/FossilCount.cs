...
public class FossilCount
{
    ...
    {
        var pouch = new InventoryPouch8(InventoryType.Treasure, ItemStorage8SWSH.Instance, 999, 0, ItemStorage8SWSH.Treasure.Length);
        pouch.GetPouch(itemsBlock);
        return pouch;
    }

    public (int, ushort) PossibleRevives(FossilSpecies f) =>
    f switch
    {
        Dracozolt => (Math.Min(Bird, Drake), (ushort)(Math.Min(Bird, Drake) == Bird ? 1105 : 1107)),
        Arctozolt => (Math.Min(Bird, Dino), (ushort)(Math.Min(Bird, Dino) == Bird ? 1105 : 1108)),
        Dracovish => (Math.Min(Fish, Drake), (ushort)(Math.Min(Fish, Drake) == Fish ? 1106 : 1107)),
        Arctovish => (Math.Min(Fish, Dino), (ushort)(Math.Min(Fish, Dino) == Fish ? 1106 : 1108)),
