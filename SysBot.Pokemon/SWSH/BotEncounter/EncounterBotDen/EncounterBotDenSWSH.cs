...
public class EncounterBotDenSWSH(PokeBotState cfg, PokeTradeHub<PK8> hub) : EncounterBotSWSH(cfg, hub)
{
    ...
    protected override async Task EncounterLoop(SAV8SWSH sav, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (Hub.Config.StopConditions.SearchConditions.Any(sc => sc.IsEnabled && sc.StopOnSpecies != Species.None))
            {
    ...
    private async Task FindSpecies(ulong targetSeed, ITrainerInfo sav, CancellationToken token)
    {
        ...
        do
        {
            ...
            if (Hub.Config.StopConditions.SearchConditions.Any(sc => sc.IsEnabled && sc.StopOnSpecies == (Species)species))
            {
