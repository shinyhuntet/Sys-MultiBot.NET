...
public sealed class OverworldBotSWSH : EncounterBotSWSH
{
    private readonly IDumper DumpSetting;
    private new readonly OverworldSettings Settings;
    private readonly byte[] BattleMenuReady = { 0, 0, 0, 255 };

    public OverworldBotSWSH(PokeBotState cfg, PokeTradeHub<PK8> hub) : base(cfg, hub)
    {
        Settings = hub.Config.EncounterSWSH.OverworldSWSH;
    ...
    {
        var task = Hub.Config.EncounterSWSH.OverworldSWSH.ScanType switch
        {
    ...
    private async Task OffsetScan(CancellationToken token)
    {
        if (Settings.NavigationType == NavigationType.Teleportation)
        {
            ...
            var ofs = await SwitchConnection.PointerAll(Offsets.PlayerCoordsPointer, token).ConfigureAwait(false);
    ...
    private async Task DoOverworldScan(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            while (DateTime.Now - start < time)
            {
                ...
                if (Hub.Config.StopConditions.SearchConditions.Where(e => e.IsEnabled).All(f => f.ShinyTarget == TargetShinyType.NonShiny || f.ShinyTarget == TargetShinyType.DisableOption))
                {
                ...
                if (Hub.Config.StopConditions.SearchConditions.Any(f => f.IsEnabled && f.ShinyTarget == TargetShinyType.AnyShiny))
                {
                    if (Settings.NavigationType == NavigationType.Fishing)
                    {
            ...
            await RolloverCorrectionFast(token).ConfigureAwait(false);
    ...
    private async Task<bool> HandleOverworldField(PK8 pk, CancellationToken token)
    {
        ...
        if (pk.ShinyXor != 0)
        {
            ...
            await WebHook.SendNotification(pk, "Overworld Shiny Encountered!", token).ConfigureAwait(false);
        ...
        if (pk.Species == (int)Species.Sinistea)
        {
            ...
            {
                Hub.LogEmbed(pk, true);
                await WebHook.SendNotification(pk, "One shiny antique teacup found... FOUND?!", token).ConfigureAwait(false);
                Log($"Pressing X to freeze Overworld spawns!");
                await Click(X, 0_600, token).ConfigureAwait(false);
        ...
        if (Settings.Conditions.StopOnBrilliantOrZeroStat)
        {
            ...
            if (brilliant || zerostat)
            {
                ...
                if (hasAMark)
                {
                    ...
                    Hub.LogEmbed(pk, true);
                    await WebHook.SendNotification(pk, $"{bmsg}{zmsg} match found on Scan: {overworldCount}!\n", token).ConfigureAwait(false);
        ...
        string[] omark = Hub.Config.EncounterSWSH.OverworldSWSH.Conditions.UnwantedMarks.Replace("Mark", "").Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
        string wmark = $"{amark.ToString().Replace("Mark", "")}";
        bool result = !omark.Contains(wmark);
        if (!result)
        {
            Hub.LogEmbed(pk, false);
            await WebHook.SendNotification(pk, "Unwanted Mark", token).ConfigureAwait(false);
        }
        ...
        if (monlist.Length != 0)
        {
            ...
            if (huntedspecies)
            {
                bool yescondition = StopConditionSettings.EncounterFound(pk, Hub.Config.StopConditions);
                if (yescondition)
                {
                    Hub.LogEmbed(pk, true);
                    await WebHook.SendNotification(pk, $"Match found on Scan: {overworldCount}!", token).ConfigureAwait(false);
                    Log($"Pressing X to freeze Overworld spawns!");
                    await Click(X, 0_600, token).ConfigureAwait(false);
        ...
        if (monlist.Length == 0)
        {
            bool matchfound = StopConditionSettings.EncounterFound(pk, Hub.Config.StopConditions) && result;
        ...
        Hub.LogEmbed(pk, false);
        await WebHook.SendNotification(pk, "Unwanted Match", token).ConfigureAwait(false);
        return false;
    }
    ...
    private async Task<bool> HandleNonShiny(PK8 pk, CancellationToken token)
    {
        ...
        bool matchfound = StopConditionSettings.EncounterFound(pk, Hub.Config.StopConditions);
    ...
    private async Task RolloverCorrectionFast(CancellationToken token, bool gameClosed = false)
    {
        if (!gameClosed)
            await Click(HOME, 2_000, token).ConfigureAwait(false);

        await TimeSkipBwd(Settings.RollBackTime, 0_400, token).ConfigureAwait(false);
    ...
    private new async Task FleeToOverworld(CancellationToken token)
    {
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    {
        ofs = await SwitchConnection.PointerAll(Offsets.PlayerCoordsPointer, token).ConfigureAwait(false);
    ...
    {
        ofs = await SwitchConnection.PointerAll(Offsets.PlayerCoordsPointer, token).ConfigureAwait(false);
        var ydown = (ushort)Hub.Config.EncounterSWSH.OverworldSWSH.MovementConditions.MoveDownMs;
        var coord = BitConverter.ToUInt32(await SwitchConnection.ReadBytesAbsoluteAsync(ofs, 4, token).ConfigureAwait(false), 0);
    ...
    {
        var ydown = (ushort)Hub.Config.EncounterSWSH.OverworldSWSH.MovementConditions.MoveDownMs;
        var yup = (ushort)Hub.Config.EncounterSWSH.OverworldSWSH.MovementConditions.MoveUpMs;
        if (await IsInBattle(token).ConfigureAwait(false))
        {
            Log($"Unwanted encounter! Initializing {Settings.Conditions.UnwantedEncounterCorrection}!");
            var task = Settings.Conditions.UnwantedEncounterCorrection switch
    ...
    {
        var yright = (ushort)Hub.Config.EncounterSWSH.OverworldSWSH.MovementConditions.MoveRightMs;
        var yleft = (ushort)Hub.Config.EncounterSWSH.OverworldSWSH.MovementConditions.MoveLeftMs;
        if (await IsInBattle(token).ConfigureAwait(false))
        {
            Log($"Unwanted encounter! Initializing {Settings.Conditions.UnwantedEncounterCorrection}!");
            var task = Settings.Conditions.UnwantedEncounterCorrection switch
    ...
    {
        var task = Hub.Config.EncounterSWSH.OverworldSWSH.NavigationOrder switch
        {
    ...
    private async Task SelectBall(CancellationToken token)
    {
        ...
        var ofs = await SwitchConnection.PointerAll(Offsets.CurrentBallIndexPointer, token).ConfigureAwait(false);
        while (true)
        {
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    public async Task<PK8?> NonShinyScan(uint offset, CancellationToken token)
    {
        ...
        if (Hub.Config.EncounterSWSH.OverworldSWSH.NavigationType == NavigationType.Teleportation || Hub.Config.EncounterSWSH.OverworldSWSH.NavigationType == NavigationType.RunDownTpUp)
            await Click(X, 1_000, token).ConfigureAwait(false);
    ...
    public async Task<PK8?> ShinyScan(uint startoffset, SAV8SWSH TrainerData, CancellationToken token)
    {
        ...
        if (shinytype == 1)
        {
            ...
            if (Version == GameVersion.SW)
                pk.Version = GameVersion.SW;
            if (Version == GameVersion.SH)
                pk.Version = GameVersion.SH;
