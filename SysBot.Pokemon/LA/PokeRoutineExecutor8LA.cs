...
public abstract class PokeRoutineExecutor8LA : PokeRoutineExecutor<PA8>
{
    ...
    public async Task PressAndHoldAndRelaese(SwitchButton b, SwitchButton c, int hold, int delay, int addelay, CancellationToken token)
    {
        await PressAndHold(b, 0, token).ConfigureAwait(false);
        await PressAndHold(c, hold, token).ConfigureAwait(false);
        await ReleaseHold(b, delay, token).ConfigureAwait(false);
        await ReleaseHold(c, addelay, token).ConfigureAwait(false);
    }
