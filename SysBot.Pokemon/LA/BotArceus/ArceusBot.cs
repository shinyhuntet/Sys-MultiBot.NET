    ...
    public sealed class ArceusBot : PokeRoutineExecutor8LA, IEncounterBot
    {
        ...
        public override async Task MainLoop(CancellationToken token)
        {
            ...
            try
            {
                ...
                if (Settings.BotType != ArceusMode.PlayerCoordScan && Settings.BotType != ArceusMode.GenieScanner && Settings.BotType != ArceusMode.OtherLegendReset && Settings.BotType != ArceusMode.AdvanceTimeDelay)
                ...
                var task = Hub.Config.ArceusLA.BotType switch
                {
                    ...
                    ArceusMode.SeedAdvancer => SeedAdvancer(dex, token),
                    ArceusMode.TimeSeedAdvancer => TimeSeedAdvancer(dex, token),
                    ArceusMode.StaticAlphaScan => ScanForSpawners(dex, token),
                    ...
                    ArceusMode.GenieScanner => ScanForSpawners(dex, token),
                    ArceusMode.OtherLegendReset => ScanForSpawners(dex, token),
        ...
        {
            var timeofs = await SwitchConnection.PointerAll(Offsets.TimePtrLA, token).ConfigureAwait(false);
        ...
        private async Task EnableInvincibleCheat(CancellationToken token)
        {
        ...
        private async Task EnableInfinitePPCheat(CancellationToken token)
        {
        ...
        {
            var ofs = await SwitchConnection.PointerAll(Offsets.PlayerCoordPtrLA, token).ConfigureAwait(false);
        ...
        private async Task Reposition(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                var wait = Settings.SpecialConditions.WaitMsBetweenTeleports;
                Settings.SpecialConditions.WaitMsBetweenTeleports = 0_800;
                await TeleportToCampZone(token);
                Settings.SpecialConditions.WaitMsBetweenTeleports = wait;
                for (int i = 0; i < 2; i++)
                ...
                wait = Settings.SpecialConditions.WaitMsBetweenTeleports;
                Settings.SpecialConditions.WaitMsBetweenTeleports = 0_800;
                await TeleportToCampZone(token);
                Settings.SpecialConditions.WaitMsBetweenTeleports = wait;
                for (int i = 0; i < 2; i++)
                    await Click(B, 0_500, token).ConfigureAwait(false);
                await SetStick(LEFT, 0, 5_000, 0_200, token).ConfigureAwait(false); // reset face forward
                ...
                for (int i = 0; i < 2; i++)
                    await Click(B, 0_500, token).ConfigureAwait(false);
                await SetStick(LEFT, 5_000, 0, 0_200, token).ConfigureAwait(false); // reset face forward
                await ResetStick(token).ConfigureAwait(false); // reset
                for (int i = 0; i < 2; i++)
                    await Click(B, 0_500, token).ConfigureAwait(false);
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (group_seed != 0)
                    {
                        ...
                        {
                            if (Settings.SpeciesToHunt.Count != 0)
                            {
                                bool huntedspecies = Settings.SpeciesToHunt.Contains((Species)match.Species);
                                if (!huntedspecies)
                                {
        ...
        private async Task TimeSeedAdvancer(PokedexSaveData dex, CancellationToken token)
        {
            int success = 0;
            int heal = 0;
            if (Settings.AlphaScanConditions.Advances <= 0)
                Settings.AlphaScanConditions.Advances = 100;
            var (Spawn, Encounter) = GetStrings();
            ...
            for (int a = 0; a < coords.Length; a++)
            {
                if (string.IsNullOrEmpty(coords[a]))
                {
                    ...
                    var encmap = JsonConvert.DeserializeObject<Dictionary<string, EncMapData>>(Spawn) ?? new();
                    var Location = encmap[$"{Settings.SpawnerID}"].Coords;
                    if (Location.All(x => x != 0))
                    {
                        Log($"Xcoord: {Location[0]}, Ycoord: {Location[1]}, Zcoord: {Location[2]}");
                        byte[] X1 = BitConverter.GetBytes(Location[0]);
                        byte[] Y1 = BitConverter.GetBytes(Location[1]);
                        byte[] Z1 = BitConverter.GetBytes(Location[2]);
                        X1 = X1.Concat(Y1).Concat(Z1).ToArray();
                        await TeleportToSpawnZoneInitial(X1, token).ConfigureAwait(false);

                    }
                    else
                    {
                        Log("Invalid spawn location data !");
                        return;
                    }
                    GetDefaultCampCoords();
                    if (Settings.AlphaScanConditions.AutoFillCoords != ArceusAutoFill.SpawnZone)
                        Settings.AlphaScanConditions.AutoFillCoords = ArceusAutoFill.SpawnZone;
            ...
            if (Settings.UsePlayerInfinitePPCheat)
                await EnableInfinitePPCheat(token).ConfigureAwait(false);

            var SpawnerOffpoint = new long[] { 0x42a6ee0, 0x330, 0x70 + Settings.SpawnerID * 0x440 + 0x20 };
            var SpawnerOff = await SwitchConnection.PointerAll(SpawnerOffpoint, token).ConfigureAwait(false);
            while (!token.IsCancellationRequested)
            {
                GetDefaultCampCoords();
                string alpha = string.Empty;
                int InitialAdvances = Settings.AlphaScanConditions.Advances;
                for (int adv = 0; Settings.AlphaScanConditions.Advances > 0; adv++)
                {
                    Log($"Advancing {InitialAdvances - Settings.AlphaScanConditions.Advances} times...");
                    ...
                    if (overworldcheck == 0)
                    {
                        ulong ofs = await SwitchConnection.PointerAll(Offsets.WildPokemonPtrLA, token).ConfigureAwait(false);
                    ...
                    var group_seed = (BitConverter.ToUInt64(GeneratorSeed, 0) - 0x82A2B175229D6A5B) & 0xFFFFFFFFFFFFFFFF;
                    Log($"{Environment.NewLine}Group seed is {group_seed:X}");
                    var advances = GenerateNextMatch(Hub.Config, false, group_seed, Spawn, Encounter, dex: dex);
                    Settings.AlphaScanConditions.Advances = advances;
                }
            }
            Log("Advances reached! Shiny should be on the next respawn! Refer to the live map if unsure of frame!");
            await Click(HOME, 1_000, token).ConfigureAwait(false);
        }
        private async Task SeedAdvancer(PokedexSaveData dex, CancellationToken token)
        {
            PokeTradeHubConfig config = Hub.Config;
            var (Spawn, Encounter) = GetStrings();
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                for (int adv = 0; Settings.AlphaScanConditions.Advances > 0; adv++)
                {
                    ...
                    if (Settings.AlphaScanConditions.SpawnIsStaticAlpha)
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                            await Click(A, 1_000, token).ConfigureAwait(false);
                        if (Settings.AlphaScanConditions.Advances % 2 != 0)
                            await Click(DDOWN, 0_500, token).ConfigureAwait(false);
                        if (Settings.AlphaScanConditions.Advances % 2 == 0)
                        {
                            for (int i = 0; i < 2; i++)
                    ...
                    await ResetStick(token).ConfigureAwait(false);
                    await TeleportToSpawnZone(token).ConfigureAwait(false);
                    var Coords = await GetPokemonCoords(token).ConfigureAwait(false);
                    for (int i = 0; i < 2; i++)
                    ...
                    Log("Trying to enter battle!");
                    await PressAndHoldAndRelaese(ZL, ZR, 0_500, 0_800, 0, token).ConfigureAwait(false);
                    await ResetStick(token).ConfigureAwait(false);
                    var overworldcheck = BitConverter.ToUInt16(await SwitchConnection.ReadBytesAbsoluteAsync(OverworldOffset, 2, token).ConfigureAwait(false), 0);
                    if (overworldcheck == 1)
                    {
                        ...
                        while (overworldcheck == 1)
                        {
                            if (tries >= 2 & tries < 5)
                            {
                                Log("Tried 2 more times, is the encounter present?.");
                                /*await TeleportToCampZone(token).ConfigureAwait(false);
                                await ForceTeleportPokemon(Coords, token).ConfigureAwait(false);
                                await TeleportToSpawnZone(token).ConfigureAwait(false);*/
                                await ForceTeleportPokemonAuto(Coords, token).ConfigureAwait(false);
                            }
                            else if (tries >= 5)
                            {
                                Log("Tried 5 more times, is the encounter present? Going back to camp to try again.");
                                await TeleportToCampZone(token).ConfigureAwait(false);
                                break;
                            }
                            await PressAndHoldAndRelaese(ZL, ZR, 0_500, 0_800, 0, token).ConfigureAwait(false);
                    ...
                    ulong ofs = await SwitchConnection.PointerAll(Offsets.WildPokemonPtrLA, token).ConfigureAwait(false);
                    var pk = await ReadPokemon(ofs, 0x168, token).ConfigureAwait(false);
                    if (overworldcheck == 0 && pk.Species > 0 && pk != null && (Species)pk.Species < Species.MAX_COUNT)
                    {
                        pk = await ReadPokemon(ofs, 0x168, token).ConfigureAwait(false);
                        if (pk != null)
                        {
                    ...
                    await TeleportToCampZone(token);
                    await SetStick(LEFT, 0, 10_000, 0_500, token).ConfigureAwait(false);
                    await ResetStick(token).ConfigureAwait(false); // reset
                    await ArceusSaveGame(token).ConfigureAwait(false);
                    var GeneratorSeed = await SwitchConnection.ReadBytesAbsoluteAsync(SpawnerOff, 8, token).ConfigureAwait(false);
                    ...
                    var group_seed = (BitConverter.ToUInt64(GeneratorSeed, 0) - 0x82A2B175229D6A5B) & 0xFFFFFFFFFFFFFFFF;
                    Log($"{Environment.NewLine}Group seed is {group_seed:X}");
                    var advances = GenerateNextMatch(config, false, group_seed, Spawn, Encounter, dex: dex);
                    Settings.AlphaScanConditions.Advances = advances;
                }
                Log("Advances reached! Target should be on the next respawn! Refer to the live map if unsure of frame!");
                return;
            }
            await Click(HOME, 1_000, token).ConfigureAwait(false);
        }
        ...
        private void GetDefaultCampCoords()
        {
            ...
            switch (mode)
            {
                ...
                case ArceusMap.AlabasterIcelands: coordinates = ("440569D2", "41F8F370", "4463EC10"); break;
            }
        ...
        private async Task ScanForSpawners(PokedexSaveData dex, CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                switch (Settings.BotType)
                {
                    case
                        ...
                        await SpawnerScan(dex, token).ConfigureAwait(false); break;
                    case
        ...
        private async Task TeleportToSpawnZoneInitial(byte[] coords, CancellationToken token)
        {
            var ofs = await SwitchConnection.PointerAll(Offsets.PlayerCoordPtrLA, token).ConfigureAwait(false);
            await SwitchConnection.WriteBytesAbsoluteAsync(coords, ofs, token).ConfigureAwait(false);
        ...
        private async Task GenieScan(CancellationToken token)
        {
            ...
            while (!isSpawner)
            {
                ...
                var group_seed = (BitConverter.ToUInt64(GeneratorSeed, 0) - 0x82A2B175229D6A5B) & 0xFFFFFFFFFFFFFFFF;
                var (Spawn, Encounter) = GetStrings();
                GenerateNextMatch(Hub.Config, true, group_seed, Spawn, Encounter, 0);
            }
        }
        ...
        private async Task SpawnerScan(PokedexSaveData dex, CancellationToken token)
        {
            await Task.Delay(0_500, token).ConfigureAwait(false);
            ...
            {
                ArceusMap.ObsidianFieldlands => 304,
                ArceusMap.CobaltCoastlands => 408,
                ArceusMap.CrimsonMirelands => 400,
                ArceusMap.CoronetHighlands => 262,
                ArceusMap.AlabasterIcelands => 297,
                _ => throw new NotImplementedException("Invalid scan location."),
            };
            ...
            for (int i = 0; i < alphacount; i++)
            {
                ...
                var (Spawn, Encounter) = GetStrings();
                GenerateNextMatch(Hub.Config, true, group_seed, Spawn, Encounter, (uint)i, dex: dex);
            }
        }
        ...
        private (List<PA8>, List<PA8>) ReadMMOSeed(PokedexSaveData dex, int totalspawn, ulong group_seed, int bonuscount, ulong encslot, ulong bonusencslot)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                var gen = ArceusRNG.GenerateFromSeed(fixedseed, rolls, poke.Item2, gt);
            ...
            for (int r = 0; r < totalspawn - 4; r++)
            {
                ...
                var gen = ArceusRNG.GenerateFromSeed(fixed_seed, rolls, poke.Item2, gt);
                int[] pkIVList = gen.IVs;
                poke.Item1.SetIVs(pkIVList);
                (pkIVList[5], pkIVList[3], pkIVList[4]) = (pkIVList[3], pkIVList[4], pkIVList[5]);
                poke.Item1.IVs = pkIVList;
        ...
        private List<PA8> ReadOutbreakSeed(PokedexSaveData dex, Species species, int totalspawn, ulong groupseed)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                var gen = ArceusRNG.GenerateFromSeed(fixedseed, rolls, givs, gt);
            ...
            for (int r = 0; r < totalspawn - 4; r++)
            {
                ...
                var gen = ArceusRNG.GenerateFromSeed(fixed_seed, rolls, givs, gt);
                pk.Species = (ushort)species;
                pk.EncryptionConstant = gen.EC;
                pk.PID = gen.PID;
                int[] pkIVList = gen.IVs;
        ...
        private void GenerateCaveFamily(bool mana, ulong seed)
        {
            ...
            for (int i = 0; i < Settings.SpecialConditions.MaxAdvancesToSearch; i++)
            {
                ...
                rng.Next();
                var givs = mana ? 3 : 0;
                var gen = ArceusRNG.GenerateFromSeed(rng.Next(), 1, givs, 0);
                if (mana)
        ...
        public async Task<byte[]> GetPokemonCoords(CancellationToken token)
        {
            var SpawnPokemonPointer = new long[] { 0x4268ee0, 0x330, 0x70 + Settings.SpawnerID * 0x440 };
            var PokemonCoordOfs = await SwitchConnection.PointerAll(SpawnPokemonPointer, token).ConfigureAwait(false);
            var Coords = SwitchConnection.ReadBytesAbsoluteAsync(PokemonCoordOfs, 12, token).Result;
            return Coords;
        }
        public async Task ForceTeleportPokemon(byte[] Coords, CancellationToken token)
        {
            var SpawnPokemonPointer = new long[] { 0x4268ee0, 0x330, 0x70 + Settings.SpawnerID * 0x440 };
            var PokemonCoordOfs = await SwitchConnection.PointerAll(SpawnPokemonPointer, token).ConfigureAwait(false);
            var PokemonX = BitConverter.ToSingle(Coords.AsSpan()[..4]);
            var PokemonY = BitConverter.ToSingle(Coords.AsSpan()[4..8]);
            var PokemonZ = BitConverter.ToSingle(Coords.AsSpan()[8..]);
            Log($"Force Teleport Spawn Pokemon to default coords.{Environment.NewLine}Coords Info: {PokemonX}, {PokemonY}, {PokemonZ}");
            await SwitchConnection.WriteBytesAbsoluteAsync(Coords, PokemonCoordOfs, token).ConfigureAwait(false);

            await Task.Delay(Settings.SpecialConditions.WaitMsBetweenTeleports, token).ConfigureAwait(false);

        }
        public async Task ForceTeleportPokemonAuto(byte[] Coords, CancellationToken token)
        {
            var SpawnPokemonPointer = new long[] { 0x4268ee0, 0x330, 0x70 + Settings.SpawnerID * 0x440 };
            var PokemonX = BitConverter.ToSingle(Coords.AsSpan()[..4]);
            var PokemonY = BitConverter.ToSingle(Coords.AsSpan()[4..8]);
            ...
            Log($"Force Teleport Spawn Pokemon to default coords.{Environment.NewLine}Coords Info: {PokemonX}, {PokemonY}, {PokemonZ}");
            for (int i = 0; i < 15; i++)
                await SwitchConnection.PointerPoke(Coords, SpawnPokemonPointer, token).ConfigureAwait(false);

            await Task.Delay(Settings.SpecialConditions.WaitMsBetweenTeleports, token).ConfigureAwait(false);

        }
        private (string, string) GetStrings()
        {
            string Spawnersjson = string.Empty;
            string Encountersjson = string.Empty;
            var mapstr = $"{Settings.SpecialConditions.ScanLocation}".ToLower();
            ...
            return (Spawnersjson, Encountersjson);
        }
        public int GenerateNextMatch(PokeTradeHubConfig config, bool scanmode, ulong seed, string Spawnersjson = "", string Encountersjson = "", uint spawnerid = 0, ulong seed1 = 0x82A2B175229D6A5B, PokedexSaveData? dex = null)
        {
            var advance = -1;
            var pk = new PA8();
            var mainrng = new Xoroshiro128Plus(seed);
            ...
            if (Settings.BotType == ArceusMode.GenieScanner || Settings.BotType == ArceusMode.OtherLegendReset)
            {
                ...
                else if (Settings.BotType == ArceusMode.OtherLegendReset)
                {
                ...
                pk.Species = (ushort)species;
                pk.Form = 0;
                int Genderratio = PersonalTable.LA.GetFormEntry(pk.Species, pk.Form).Gender;
                ...
                for (int i = 0; i <= Settings.SpecialConditions.MaxAdvancesToSearch; i++)
                {
                    ...
                    var gen = ArceusRNG.GenerateFromSeed(rng.Next(), 1, 3, Genderratio);
                    pk.EncryptionConstant = gen.EC;
                    pk.PID = gen.PID;
            ...
            {
                var GauranteedIVs = 0;
                var EncMap = JsonConvert.DeserializeObject<Dictionary<string, EncMapData>>(Spawnersjson) ?? new();
                var SpawnMap = JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, Dictionary<string, float>>>>(Encountersjson) ?? new();
                var Search_Index = scanmode ? spawnerid : Settings.SpawnerID;
                var GroupName = EncMap[$"{Search_Index}"].Name;
                var Spawns = SpawnMap[$"{GroupName}"];
                var SpawnDict = GetSlotDetails(Spawns);
                var PossibleSpawns = SpawnDict.ElementAt(0).Value;
                if (!SpawnDict.ContainsKey(Settings.TimesAndWeather))
                {
                    Log($"Key: {Settings.TimesAndWeather} is not contained by Data");
                    return -1;
                }
                float EncSlotSum = GetTotalSlot(PossibleSpawns);
                if (Settings.AlphaScanConditions.CheckforEncSlot)
                {
                    PossibleSpawns = SpawnDict[Settings.TimesAndWeather];
                }
                (float EncMin, float EncMax) = (0, 0);
                var EncMaxList = new List<float>();
                var EncMinList = new List<float>();
                int Targetindex = -1;
                List<int> IndexList = new();
                if (Hub.Config.ArceusLA.TargetSpecies != Species.None)
                {
                    (EncMin, EncMax) = GetEncMinMax((ushort)Hub.Config.ArceusLA.TargetSpecies, Settings.Alpha, PossibleSpawns, out Targetindex);
                ...
                else if (Settings.Alpha)
                {
                    ...
                    {
                        (EncMin, EncMax) = GetAlphaEncMinMax(PossibleSpawns, IndexList, out Targetindex);
                        if (Targetindex != -1)
                        {
                            EncMinList.Add(EncMin);
                            EncMaxList.Add(EncMax);
                            IndexList.Add(Targetindex);
                        }
                    }
                }
                var Advances = new List<int>();
                var Result = new List<(ulong, PA8)>();
                ...
                for (int i = 0; i <= Settings.SpecialConditions.MaxAdvancesToSearch; i++)
                {
                    ...
                    mainrng.Next();
                    var rng = new Xoroshiro128Plus(generator_seed);
                    float EncounterSlotRand = (float)(EncSlotSum * (float)((float)rng.Next() * 5.421e-20f)) + 0;
                    PokeSlotDetail slot = new();
                    if (Targetindex == -1 && IndexList.Count == 0)
                    {
                        slot = GetSlots(PossibleSpawns, EncounterSlotRand);
                        CorrectSlot = true;
                        AlphaMatch = slot.IsAlpha;
                    }
                    else
                    {
                        if (IndexList.Count == 0)
                        {
                            if (EncMin <= EncounterSlotRand && EncounterSlotRand < EncMax)
                            {
                                slot = PossibleSpawns[Targetindex];
                                CorrectSlot = true;
                                if (slot.IsAlpha)
                                    AlphaMatch = true;
                            }
                        ...
                        else
                        {
                            ...
                            {
                                if (EncMinList[j] <= EncounterSlotRand && EncounterSlotRand < EncMaxList[j])
                                {
                                    slot = PossibleSpawns[IndexList[j]];
                                    CorrectSlot = true;
                                    AlphaMatch = slot.IsAlpha;
                                }
                            }
                    ...
                    if (AlphaMatch || Settings.AlphaScanConditions.SpawnIsStaticAlpha)
                        GauranteedIVs = 3;
                    else
                        GauranteedIVs = 0;
                    ...
                    var (perfect, complete) = CheckForPerfectComplete(HasCharm, dex!, pk.Species);
                    var Rolls = 1 + (complete ? 1 : 0) + (perfect ? 2 : 0) + (HasCharm ? 3 : 0);

                    (pk, var newseed) = ArceusRNG.GeneratePKMFromSeed(rng.Next(), Rolls, GauranteedIVs, pk);

                    if (StopConditionSettings.EncounterFound(pk, config.StopConditions))
                    {
                        ResultStrings.Add(GetFormattedShowdownText(pk));
                        Log("Encounter found!");
                        if (scanmode)
                            Settings.AlphaScanConditions.StopOnMatch = true;
                        Advances.Add(i);
                        Result.Add((generator_seed, pk));
                        if (Advances.Count >= 3)
                            break;
                    }
                }
                if (Advances.Count == 0)
                {
                    Log($"No Match Found within {Settings.SpecialConditions.MaxAdvancesToSearch} Advances");
                    return -1;
                }
                advance = Advances.Min();
                var AdvIndex = Advances.IndexOf(advance);
                var result = Result[AdvIndex];
                var resultstring = ResultStrings[AdvIndex];
                Log($"Result{Environment.NewLine}GeneraterSeed: {result.Item1:08X}{Environment.NewLine}Advance: {advance}{Environment.NewLine}{resultstring}");
                return advance;
            }
            return advance;
        }

        public static string GetFormattedShowdownText(PKM pk)
        {
            string ShinyString = !pk.IsShiny ? "Nonshiny" : pk.ShinyXor == 0 ? "Square" : "Star";
            string GIVString = string.Empty;
            Gender gender = pk.Gender <= 2 ? (Gender)pk.Gender : (Gender)(pk.Gender - 2);

            if (pk is PA8 p)
                GIVString = $"\nGVs: {p.GV_HP} HP / {p.GV_ATK} Atk / {p.GV_DEF} Def / {p.GV_SPA} SpA / {p.GV_SPD} SpD / {p.GV_SPE} Spe";

            return $"\nSpecies: {(Species)pk.Species} ({gender})\nEC: {pk.EncryptionConstant:X}\nPID: {pk.PID:X}\nShiny: {ShinyString}\n" +
                $"{pk.Nature} nature\nAbility slot: {pk.AbilityNumber}\nAbility: {(Ability)pk.Ability}\n" +
                $"IVs: [{pk.IV_HP}, {pk.IV_ATK}, {pk.IV_DEF}, {pk.IV_SPA}, {pk.IV_SPD}, {pk.IV_SPE}]{GIVString}";
        }
        ...
        private (ulong, PA8) GenerateNextShiny(int spawnerid, ulong seed)
        {
            ...
            {
                string spec = Settings.SpecialConditions.ScanLocation switch
                {
                ...
                Species species = Enum.Parse<Species>(spec);
                ...
                for (int i = 0; i <= Settings.SpecialConditions.MaxAdvancesToSearch; i++)
                {
                    ...
                    var gen = ArceusRNG.GenerateFromSeed(rng.Next(), (int)Settings.AlphaScanConditions.StaticAlphaShinyRolls, givs, 0);
                    //Log($"\nAdvances: {i}\nAlpha: {species} - {gen.shinyXor} | SpawnerID: {spawnerid}\nEC: {gen.EC:X8}\nPID: {gen.PID:X8}\nIVs: {string.Join("/", gen.IVs)}\nNature: {gen.Item8}\nSeed: {gen.Item9:X16}");
                    if (gen.shiny)
                    {
                        if (Settings.SpeciesToHunt.Count != 0 && !Settings.SpeciesToHunt.Contains(species))
                            break;
        ...
        private async Task PerformOutbreakScan(PokedexSaveData dex, CancellationToken token)
        {
            ...
            {
                int count = 0;
                Species outbreakspecies = (Species)BitConverter.ToUInt16(info.AsSpan(0 + (i * 80), 2).ToArray());
                if (outbreakspecies != Species.None)
                {
                    var outbreakseed = BitConverter.ToUInt64(info.AsSpan(56 + (i * 80), 8).ToArray());
                    var spawncount = BitConverter.ToUInt16(info.AsSpan(64 + (i * 80), 2).ToArray());
                ...
                if (Settings.SpeciesToHunt.Contains(outbreakspecies) && Settings.OutbreakConditions.StopIfSpeciesFound)
                {
            ...
            {
                PermuteMeta.SatisfyCriteria = (result, advances) => Settings.OutbreakConditions.PermuteType switch
                {
                    PermuteMode.Shiny => result.IsShiny,
                    PermuteMode.ShAlpha => result.IsShiny && result.IsAlpha,
                    PermuteMode.Alpha => result.IsAlpha || (result.IsShiny && result.IsAlpha),
                    _ => result.IsShiny,
                };
                ...
                {
                    if (Settings.SpeciesToHunt.Contains(s))
                    {
        ...
        {
            bool huntedspecies = Settings.SpeciesToHunt.Contains((Species)pk.Species);
            ...
            if (Settings.SpeciesToHunt.Count == 0 && !Settings.OutbreakConditions.CheckBoxes)
            {
        ...
        private static void SetFakeTable(SlotDetail[] slots, ulong key)
        {
            ...
            if(SpawnGenerator.EncounterTables.ContainsKey(key))
                SpawnGenerator.EncounterTables[key] = slots;
            else
        ...
        private async Task PerformMultiSpawnerScan(PokedexSaveData dex, CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                var task = Hub.Config.ArceusLA.MultiScanConditions.MultiSpecies switch
                {
                    ...
                    _ => PerformMultiSpeciesScan(token),
                };
                await task.ConfigureAwait(false);
            }
        }
        private Dictionary<MultiSpawners, SlotDetail[]> FakeTable = new()
        {
            {
                MultiSpawners.Eevee,
...
// Fieldlands: 87F39EBD429BF90C; Not Night; Sunny, Cloudy, Intense Sun, Fog, Rainstorm (2-2)
            ...
            {
                MultiSpawners.QwilfishLeft1,
                // Coastlands: 98377DDDC282361F groupID: 341, A6E9A110EBF37C0D groupID: 343, BFDB454F1B881B09 groupID: 362, 3B2E318D82A87165 groupID: 363; Not Night; All weather (3-3)
            [
            ...
            {
                MultiSpawners.Abra,
                // Fieldlands: B829961431D954A4; groupID: 273; Not Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Shellos,
                // Fieldlands: 264C1AE897749DE3; groupID: 253; All times; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Ralts,
                // Mirelands: DFDE7E3B3FFEF5C9; groupID: 50; Not Night; Cloudy, Rain, Rainstorm (2-2)
            [
            ...
            {
                MultiSpawners.Hippopotas,
                // Mirelands: 3402DE384DC3A82A; groupID: 117; Not Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Aipom,
                // Fieldlands: 0E4264E9B8F2E85F; groupID: 247; Not Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Pikachu,
            // Fieldlands: 7F13E7A756EAECEA; groupID: 129 Not Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Petilil,
                // Mirelands: 90B205067060D0BC; groupID: 367; Not Night; Not Sunny (3-3)
            [
            ...
            {
                MultiSpawners.Gastly,
                // Mirelands: 90B205067060D0BC; groupID: 367; Night; Not Sunny (3-3)
            [
                new(100, "Petilil", false, [33, 36], 0),
                new(2, "Petilil", true, [48, 51], 3),
            ...
            {
                MultiSpawners.Glameow,
                // Coastlands: 3BC31AA6F5337A4D; groupID: 357; All times; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Teddiursa,
                // Highlands: 8A628649D2AB899C; groupID: 56; All times; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Beautifly,
                // Fieldlands: A366202BD3643B7E; groupID: 234; Not Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Dustox,
                // Fieldlands: A366202BD3643B7E; groupID: 234; Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Murkrow,
                // Coastlands: 1FB2A7A1C1FBFEBD; groupID: 161; Night; All weather (2-2)
            [
            ...
            {
                MultiSpawners.Swinub,
                // Icelands: 81AB7DC29C2E5AB3; groupID: 245; Not Night; Sunny, Cloudy (2-2)
            [
            ...
            {
                MultiSpawners.Paras,
                // Mirelands: A3C27A3A01165FE6; groupID: 182; Not Night; Cloudy, Rain, Fog, Rainstorm (2-2)
            [
            ...
            {
                MultiSpawners.Zubat,
                // Mirelands: A3C27A3A01165FE6; groupID: 182; Night; Cloudy, Rain, Fog, Rainstorm (2-2)
            [
                new(100, "Paras", false, [20, 23], 0),
                new(2, "Paras", true, [35, 38], 3),
                new(50, "Parasect", false, [25, 28], 0),
            ...
            {
                MultiSpawners.Rufflet,
                // Icelands: F4A47B912D22A05A groupID: 203, F91217341C696F1F groupID: 206; All times; All weather (2-2)
            [
        ...
        private (ulong, int) GetKeyCount(SlotDetail[] slots, MultiSpawners spawners)
        {
            ulong key = (ulong)(slots.Any(x => x.Species == (ushort)Species.Basculin) ? 0x123456B0 : 0x694201020 + FakeTable.Keys.ToList().IndexOf(spawners));
            int count = spawners switch
            {
                MultiSpawners.QwilfishLeft1 or MultiSpawners.QwilfishLeft2 or MultiSpawners.QwilfishRight1 or MultiSpawners.QwilfishRight2 or MultiSpawners.Petilil or MultiSpawners.Gastly => 3,
                _ => 2,
            };

            return (key, count);
        }
        private async Task PerformMultiSpeciesScan(CancellationToken token)
        {
            var slots = Settings.MultiScanConditions.MultiSpecies switch
            {
                MultiSpawners.Petilil when Settings.MultiScanConditions.IsNight is false =>
                [
                    new(100, "Petilil", false, [33, 36], 0),
                    new(2, "Petilil", true, [48, 51], 3),
                ],
                MultiSpawners.Gastly when Settings.MultiScanConditions.IsNight is true =>
                [
                    new(100, "Petilil", false, [33, 36], 0),
                    new(2, "Petilil", true, [48, 51], 3),
                    new(50, "Gastly", false, [21, 24], 0),
                ...
                MultiSpawners.Paras when Settings.MultiScanConditions.IsNight is false =>
                [
                    new(100, "Paras", false, [20, 23], 0),
                    new(2, "Paras", true, [35, 38], 3),
                    new(50, "Parasect", false, [25, 28], 0),
                ...
                MultiSpawners.Zubat when Settings.MultiScanConditions.IsNight is true =>
                [
                    new(100, "Paras", false, [20, 23], 0),
                    new(2, "Paras", true, [35, 38], 3),
                    new(50, "Parasect", false, [25, 28], 0),
                ...
                _ => FakeTable[Settings.MultiScanConditions.MultiSpecies],
            };
            foreach (var s in slots)
                s.SetSpecies();
            (ulong key, int count) = GetKeyCount(slots, Settings.MultiScanConditions.MultiSpecies);
            if (SpawnGenerator.EncounterTables.ContainsKey(key))
                SpawnGenerator.EncounterTables[key] = slots;
            else
                SpawnGenerator.EncounterTables.Add(key, slots);
        ...
        private async Task PerformMMOScan(PokedexSaveData dex, CancellationToken token)
        {
            ...
            if (Settings.OutbreakConditions.Permute)
            {
                ...
                if (Settings.SpeciesToHunt.Count == 0 && specieslist.Count > 0)
                {
                    IsWaiting = true;
                    while (IsWaiting)
        ...
        private Dictionary<string, List<PokeSlotDetail>> GetSlotDetails(Dictionary<string, Dictionary<string, float>> Dict)
        {
            Dictionary<string, List<PokeSlotDetail>> Result = new();
            foreach(var KeyValuePair in Dict)
            {
                List<PokeSlotDetail> DetailsList = new();
                foreach(var kvp in KeyValuePair.Value)
                {
                    var SlotDetail = new PokeSlotDetail();
                    SlotDetail.Rate = kvp.Value;
                    SlotDetail.GetSpecies(kvp.Key);
                    SlotDetail.GetForm(kvp.Key);
                    SlotDetail.CheckIsAlpha(kvp.Key);
                    DetailsList.Add(SlotDetail);
                }
                Result.Add(KeyValuePair.Key, DetailsList);
            }
            return Result;
        }
        private float GetTotalSlot(List<PokeSlotDetail> SlotDetail)
        {
            float encmax = 0;
            foreach (var slot in SlotDetail)
                encmax += slot.Rate;
            return encmax;
        }
        private (float, float) GetEncMinMax(ushort Species, bool isAlpha, List<PokeSlotDetail> slotDetails, out int index)
        {
            float encmin = 0;
            float encmax = 0;
            index = -1;
            foreach(var slot in slotDetails)
            {
                encmin = encmax;
                if (slot.Species == Species && slot.IsAlpha == isAlpha)
                {
                    encmax += slot.Rate;
                    index = slotDetails.IndexOf(slot);
                    break;
                }
                encmax += slot.Rate;
            }
        ...
        private (float, float) GetAlphaEncMinMax(List<PokeSlotDetail> slotDetails, List<int> IndexList, out int index)
        {
            float encmin = 0;
            float encmax = 0;
            index = -1;
        ...
        private PokeSlotDetail GetSlots(List<PokeSlotDetail> slotDetails, float encrand)
        {
            float encmax = 0;
            foreach(var slot in slotDetails)
            {
    ...
    public sealed record EncMapData(
        [property: JsonProperty("coords")] List<float> Coords,
        [property: JsonProperty("icon")] string Iconurl,
        [property: JsonProperty("groupID")] int GroupID,
        [property: JsonProperty("ivs")] int GuarnteedIVs,
        [property: JsonProperty("name")] string Name)
    {

    }
    public sealed record PokeSlotDetail
    {
        ...
        public void CheckIsAlpha(string Name)
        {
        ...
        public void GetSpecies(string Name)
        {
            ...
            if(!SpeciesName.TryGetSpecies(Name, (int)LanguageID.English, out ushort temp))
                throw new Exception("Target Species Name doesn't exsit.");
            if (temp < 0)
                throw new Exception("Invalid Species ID( < 0).");
            Species = temp;
        }
    }
