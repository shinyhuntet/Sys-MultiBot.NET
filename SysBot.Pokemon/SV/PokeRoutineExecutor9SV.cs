...
public abstract class PokeRoutineExecutor9SV(PokeBotState cfg) : PokeRoutineExecutor<PK9>(cfg)
{
    ...
    public async Task<(bool, int)> GetPartyEmptySlot(CancellationToken token)
    {
        int slot;
        for (slot = 0; slot < 6; slot++)
        {
            (var pk9, _) = await ReadRawPartyPokemon(slot, token).ConfigureAwait(false);
            if (!PersonalTable.SV.IsPresentInGame(pk9.Species, pk9.Form) || pk9 == null || pk9.Species <= 0 || pk9.Species > (ushort)Species.MAX_COUNT || !pk9.ChecksumValid || !pk9.Valid)
                return (true, slot);
        }
        return (false, -1); // No empty slot
    }
