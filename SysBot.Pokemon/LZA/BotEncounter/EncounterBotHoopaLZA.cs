...
public class EncounterBotHoopaLZA(PokeBotState cfg, PokeTradeHub<PA9> hub) : EncounterBotLZA(cfg, hub)
{
    private readonly ushort _hoopa = (ushort)Species.Hoopa;
    ...
    protected override async Task EncounterLoop(SAV9ZA sav, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            while (pa9 == null || pa9.Species == 0 || !pa9.Valid || pa9.EncryptionConstant == 0)
            {
                ...
                if (pa9.Species > 0 && pa9.Species != _hoopa)
                {
                    await dialogueCancellationTokenSource.CancelAsync();
                    Log($"Detected species {(Species)pa9.Species}, which shouldn't be possible. Only 'none' or 'Hoopa' are expected");
                    return;
                }

                if (pa9.Species == _hoopa)
                {
