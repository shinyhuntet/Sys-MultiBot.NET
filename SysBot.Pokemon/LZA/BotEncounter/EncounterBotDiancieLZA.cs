...
public class EncounterBotDiancieLZA(PokeBotState cfg, PokeTradeHub<PA9> hub) : EncounterBotLZA(cfg, hub)
{
    ...
    {
        var (EmptyBox, EmptyBoxSlot) = await ReadEmptySlot(0, 0, token).ConfigureAwait(false);
        var (box, boxSlot) = AdjustBoxSlot(EmptyBox, EmptyBoxSlot);
        var (EmptyBox2, EmptyBoxSlot2) = await ReadEmptySlot(box, boxSlot, token).ConfigureAwait(false);
        (box, boxSlot) = AdjustBoxSlot(EmptyBox2, EmptyBoxSlot2);
        var (EmptyBox3, EmptyBoxSlot3) = await ReadEmptySlot(box, boxSlot, token).ConfigureAwait(false);
        List<(int, int)> EmptySlots =
        [
            (EmptyBox, EmptyBoxSlot),
            (EmptyBox2, EmptyBoxSlot2),
            (EmptyBox3, EmptyBoxSlot3)
        ];
        foreach ( var slot in EmptySlots )
        {
            if(slot.Item1 == -1 || slot.Item2 == -1)
            {
                Log("No more empty slots available. Stopping the bot.");
                return;
            }
        }
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            while (result != EncounterResult.DiancieFound && DateTime.Now <= later)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    result = await LookupSlot(EmptySlots[i].Item1, EmptySlots[i].Item2, token).ConfigureAwait(false);
                    ...
                    {
                        Log($"Diancie found in B{EmptySlots[i].Item1 + 1}S{EmptySlots[i].Item2 + 1}, rebooting the game...");
                        break;
                    }
                }
            }
    ...
    private (int, int) AdjustBoxSlot(int box, int slot)
    {
        slot++;
        if(slot == 30)
        {
    ...
    private async Task<EncounterResult> LookupSlot(int box, int slot, CancellationToken token)
    {
        (var pa9, var raw) = await ReadRawBoxPokemon(box, slot, token).ConfigureAwait(false);
